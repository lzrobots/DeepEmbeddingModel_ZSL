...
# # Network
# CUB 312 300 2048 ReLu, 1e-2 * regularisers, 64 batch, 0.00001 Adam
W_left_a1 = weight_variable([312, 1200])
b_left_a1 = bias_variable([1200])
left_a1 = tf.nn.relu(tf.matmul(att_features, W_left_a1) + b_left_a1)


W_left_a2 = weight_variable([1200, 2048])
b_left_a2 = bias_variable([2048])
left_a2 = tf.nn.relu(tf.matmul(left_a1, W_left_a2) + b_left_a2)


# # loss
loss = tf.reduce_mean(tf.square(left_a2 - visual_features))

# L2 regularisation for the fully connected parameters.
regularisers = (tf.nn.l2_loss(W_left_a1) + tf.nn.l2_loss(b_left_a1)
                + tf.nn.l2_loss(W_left_a2) + tf.nn.l2_loss(b_left_a2))
               
loss += 1e-2 * regularisers
...
for i in range(1000000):
    ...
    if i % 1000 == 0:
        ...
        acc_seen_gzsl = compute_accuracy(attribute, x_test_seen, np.arange(200), test_label_seen)
        acc_unseen_gzsl = compute_accuracy(attribute, x_test, np.arange(200), test_label)
